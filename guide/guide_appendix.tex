...
\begin{longtable}{lp{0.8\textwidth}}
  ...
  Date & Comment\\
  \midrule
  31 Mar 2013 & Add instrcutions on how to use \LaTeX\ backport under
  Ubuntu variants.\\
  ...
  \Macro{RequirePackage} in \texttt{pibonn-thesis.sty}.\\
...
produce the guide (assuming that you have \TeXLive 2009):
\begin{verbatim}
mkdir pithesis; cd pithesis
svn co https://svn.physik.uni-bonn.de/basic/pithesis/trunk
cd pithesis/trunk
\end{verbatim}
...
This default version of the guide uses \Option{bibtex8} for the
\Package{biblatex} package and includes\\
\texttt{./refs/example\_refs-latin1} rather than
\texttt{./refs/example\_refs-utf8}. You give the command \texttt{make
  guide} to compile, which uses \texttt{bibtex} rather than
\texttt{biber} to process the \texttt{.bib} files.

For a new version of \TeXLive (2011 or later) you change
\Macro{texlive} in \texttt{thesis\_guide.tex} to \texttt{2011}. Then
the following works:
\begin{verbatim}
make feynmf
make guide11
...
computers ((K)Ubuntu 12.04), with the same change to the
\Macro{texlive} version and using \texttt{make guide11}.

If you want to switch from \texttt{bibtex} to \texttt{biber} or vice
versa, give the commands \texttt{make cleanguide cleanbbl cleanblx}
before you try to compile the guide (or your thesis).

In order to edit \LaTeX{} you need an editor such as \texttt{emacs} or
use a package such as \texttt{kile} and the associated
\texttt{kile-doc}. See below for a bit more information on using \texttt{kile}.
...
\LaTeX. If you use \texttt{bibtex} for you references then the default
...
the mouse and enter\\
\texttt{https://svn.physik.uni-bonn.de/basic/pithesis/trunk} or\\
\texttt{https://svn.physik.uni-bonn.de/basic/pithesis/tags/pithesis-N.M}
as the URL.
...
instead. \TeX makerX has recently been superseded by \TeXstudio, which I
will comment on below. My comments on \TeXnicCenter have been
relegated to a subsection. There is also the package
\TeXmaker. \TeXmaker and \TeXstudio (\TeX makerX) split in 2009, so
both packages contain a lot of similarities. \TeXmaker is available as
an Ubuntu package and there are also versions for MacOSX and Windows.
...
\TeXstudio and \TeXmaker seem to be more up-to-date replacements for
\TeXnicCenter and can be installed under Windows, MacOSX or Linux. I
have tried \TeXstudio under Windows. It can be downloaded from
\url{http://texstudio.sourceforge.net/}. Others report good experience
with \TeXmaker (under Ubuntu). As mentioned above it is available as
an Ubuntu package or from \url{http://www.xm1math.net/texmaker}.
...
\texttt{make guide11} and should be added to \texttt{make thesis}
and/or \texttt{make thesis11} if you want to make a glossary for your
thesis.
...
For a PhD thesis, as mentioned in Section~\ref{sec:tips:submit:phd},
you have to print and bind the five copies that are needed by the
university library externally. Some print shops in Bonn that can supply
the required quality at a reasonable price are:
...
To do this you can start with the \texttt{feynmf} command on the
\LaTeX{} file.\footnote{Note that the example files that are included
  in this document tree are missing the \Macro{documentclass} and
  \Macro{usepackage} commands.}
The drawback is that this produces a
\texttt{dvi} file that you then have to convert to Encapsulated
Postscript and/or PDF. This is not totally trivial.
