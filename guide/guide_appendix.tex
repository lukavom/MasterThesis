...
\begin{longtable}{lp{0.8\textwidth}}
  ...
  Date & Comment\\
  \midrule
  02 Feb 2015 & Add ability to pass options to \texttt{ubonn-thesis.sty}
  using \Package{keyval} package.
  \TeXLive 2014 is now the default.
  \TeXLive version can be passed as an option. Improve its handling.
  Change a few default packages:
  \Package{longtable} $\to$ \Package{xtab};
  \Package{subfig} $\to$ \Package{subcaption} (for \TeX Live 2012 and later).
  Add options for different fonts. 
...
possible -- I recommend \TeXLive 2013 or 2014.
...
\texttt{bibtex8}\index{bibtex8} to process the \texttt{.bib} files.
...
\texttt{bibtex8}\index{bibtex8} or \texttt{bibtex}\index{bibtex} or vice versa, give the commands
...
same way). For a while,I did this as standard practice on my laptop
and it worked very well. Another useful source of information on how to
...
You can also doenload Mac\TeX from \url{http://www.tug.org/mactex/}.

There are MacOSX versions of \TeXstudio and \TeXmaker which you can
also try. I have been using \TeXstudio.
