...
in \texttt{pibonn-thesis.sty}. Some examples of the possibilities are
...
\Macro{put} command. This has the big advantage that you can display the
letters in the figure without actually having to add them to the
...
\Macro{setlength}\texttt{\{\textbackslash unitlength\}} command, which is by default
set to \SI{1}{\mm} in \texttt{pibonn-thesis.sty}. The same units
are also used for Feynman graphs made with the \Package{feynmf} and/or \Package{feynmp}
package -- see Section~\ref{sec:fig:feynman:feynmf}.
...
\Package{subfig} package is probably the way to go. You can see how to
...
\begin{figure}[htbp]
  \centering
  \subfloat[NC scattering][NC]{\label{fig:nccc-nc}\includegraphics[width=0.5\figwidth]{ep_nc}}
  \qquad
  \subfloat[CC scattering][CC]{\label{fig:nccc-cc}\includegraphics[width=0.5\figwidth]{ep_cc}}
...
This restriction have finally been lifted with \TeXLive 2011. It
...
\Package{feynmp} packages.  The NC and CC graphs shown in
...
show a charged current process. This paragraph illustrates how to
reference different parts of a figure that uses \Package{subfig}.

I explain in more detail how to use \Package{feynmf} and \Package{feynmp}
below. Instructions on how to use \Package{axodraw} may come at a later
...
Metapost.\footnote{\texttt{pibonn-thesis.sty} includes \Package{feynmf}
  by default. If you want to use \Package{feynmp} instead just change
  the name.}
You can either make a separate \LaTeX{} file containing
the Feynman graph or include the commands inside your normal \LaTeX{}
file.  The tricky bit is running Metafont. On Unix systems the
...
then look at or use to make a Postscript/PDF file.  It looks as if it
is a bit easier to use Metapost. You should be able to just run
...
\item use the \texttt{Makefile} described below to automatically run
  over one or all \Package{feynmf}/\Package{feynmp} \LaTeX{} files by giving the
  command \texttt{make feynmf} or \texttt{make feynmp}. Check
...
The \texttt{run\_mp} script works a bit differently at the moment. If
...
files) in the \texttt{feynmf} directory. The resulting PDF files are
copied back to the \texttt{feynmf} directory.
...
files in the directory \texttt{./feynmf}. If you give the command
