...
\Package{pgf} and \Package{tikz} are packages to help with creating
...
guide and a few more are collected in the \texttt{tikz} subdirectory.
...
Metapost.\footnote{\texttt{ubonn-thesis.sty} includes \Package{feynmp}
  by default. If you want to use \Package{feynmf} instead just change
...
file.
Metapost\index{metafont} is a bit
...
of the \Env{fmffile} environment.
...
With the \TeXLive setups I use to test this guide, this is all you
need. If \texttt{mpost} does not run automatically, you must execute
the command \texttt{pdflatex --shell-escape mythesis} when you
compile your thesis. This can be achieved by including the
\texttt{EXTRACMD} definition that is commented out in the
\texttt{Makefile}. In this guide I put the Feynman graph in its own
file and the \Macro{write18} statement in the main file so that I
can test \Package{feynmf} and \Package{feynmp} with the same \LaTeX\
code. In your thesis, you should probably put the \Macro{write18}
