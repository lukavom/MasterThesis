...
\Package{typearea} or the \Package{geometry}
package. In this guide I use \Package{typearea} as it is closely tied
into \KOMAScript. The basic command is to specify how many pieces to
divide the page into. This guide uses \Option{DIV=14} by default with
a binding correction of \Option{BCOR=8mm}.  You can then let
\KOMAScript{} and \Package{typearea} sort out the rest.
...
\end{verbatim}
In this guide and in \texttt{pibonn-thesis.sty} I have changed this to:
\begin{verbatim}
...
should be possible to keep the same format. If you use \Env{longtable}
rather than \Env{table} and \Env{tabular}, you can also include
footnotes easily -- see Appendix~\ref{sec:app:tables} for an
example. Another recent package (\TeXLive 2011) is
...
\texttt{pibonn-thesis.sty} is independent of the thesis language. If
...
for you in \texttt{pibonn-thesis.sty} in such a way that if you
...
usually commented out in \texttt{pibonn-thesis.sty}. An alternative
...
have set a few such fonts in \texttt{pibonn-thesis.sty}. In particular
...
alternatives to \Package{txfonts} (with the \Option{varg} option) seem
...
Having used \Package{csquotes} for a year of so now, I find it a
...
coloured dark green. These settings are in \texttt{pibonn-thesis.sty}.
...
changes. I tried out \Package{fncychap}, \Package{quotchap} and
\Package{titlesec}. A variant of \Package{titlesec} is used for this
guide and by default for the thesis. If you want to get back to the
usual style, just comment out the appropriate lines in
\texttt{pibonn-thesis.sty}. See the documentation on
