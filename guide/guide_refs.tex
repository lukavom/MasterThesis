...
If you use traditional \BibTeX, then you probably have to use the \Package{mcite}
...
setting for \TeXLive 2010 and earler), then you have to encode your \texttt{.bib}
...
cited. \Option{biber} seems to work well with \TeXLive 2012 and later;\footnote{%
I have seen occasional problems with \TeXLive 2011, but it is mostly OK.}
it is often not available with earlier versions.
\texttt{ubonn-thesis} switches to \Option{bibtex8} automatically for \TeXLive
...
2011 or earlier, and therefore the \texttt{bibtex} or \texttt{bibtex8} backend, the
...
In addition you have give the command \Macro{printbibliography[heading=subbibliography]} at the end of each chapter.
