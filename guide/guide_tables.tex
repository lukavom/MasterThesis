...
code, the \TeXLive 2009 version is inside \Macro{ifthenelse}\{\textbackslash texlive = 2009\}, while the \TeXLive 2011 version
...
\Option{tabformat}\footnote{\Option{table-format} in \TeXLive 2011} to
...
\Option{tabnumalign}\footnote{\Option{table-number-alignment} in
  \TeXLive 2011} option to change this. \Option{S} and \Macro{num}
...
the \Option{dp} option gives the number of decimal places for the
...
use options such as \Option{dp}\footnote{\Option{round-precision} in
  \TeXLive 2011} in the \Option{S} command or use the macro
...
should \Option{dp} for the rounding. In version 2 you should use the
options \Option{round-mode} and \Option{round-precision}. In the first
...
\Option{dp}\footnote{\Option{round-mode} and \Option{round-precision}
  in \TeXLive 2011. The \Option{round-mode} should be set in the
  preamble of the table and not for every number.} is shown in the
right half. Note the use of options for the \Option{S} command and the
use of \Macro{num} enclosed in braces to format the row that requires
...
mode, so \Macro{centering} is needed to get centred headers.
