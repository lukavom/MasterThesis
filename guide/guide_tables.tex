...
% The syntax of some of the
% options changed between version 1 (\(\leq 2009\)) and version 2 (\(\geq 2011\)).
% I discuss the version 2 options in the main text and give the equivalent
...
\Option{table-number-alignment} option to change this.
\Option{S} and \Macro{num} cannot generally be mixed in a single column though.
...
The spacing of the captions in \cref{tab:viscosity,tab:xsect0} is the same as
the option \Option{captions=tableheading} is set in this guide.
...
use options such as \Option{round-precision} in the \Option{S} command or use the macro
