...
document are available in a subversion repository and as a
\texttt{tar} file. To get the latest subversion entries give the
command:
...
Note that as of release 2.1, the main file for the thesis
...
Chapter~\ref{sec:submit} for some more details.
...
\TeXLive 2011. For earlier versions set \Macro{texlive} in
\texttt{thesis\_guide.tex} to 2009.\footnote{You should set
  \Macro{texlive} to 2011 also if you have \TeXLive
...
\item Make sure every table and figure is referenced in the text. I
  ...
  \Package{refcheck}. You need to run \LaTeX\ one more time for it to
...
why I use \Package{txfonts} in this guide. If you compile the guide
with a different font and the numbers $1234.56$ and 1234.56 look the
same then you do not have to worry! There are ways to get around the
problem with Palatino. You can try either the \Package{pxfonts} or the
...
using SVN, then you just need to do an \texttt{svn update}. If you use
...
Note that when you make a new thesis skeleton (as of release 2.1) the files
\texttt{thesis\_skel/Makefile} and \texttt{ubonn-thesis.sty} are copied
