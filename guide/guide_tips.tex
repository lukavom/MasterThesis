...
To get the latest Git entries give the command:
...
{\small
\begin{verbatim}
git clone --branch vN.M https://bitbucket.team.uni-bonn.de/scm/uni/ubonn-thesis.git
\end{verbatim}
}
...
\Option{exponent-product} % \footnote{\Option{expproduct} in \TeXLive 2009.}
...
\Option{separate-uncertainty} % \footnote{\Option{seperr} in \TeXLive 2009.}
option (which I specify) or \(2.88(32)\) with the default
...
\ifthenelse {\texlive < 2011} {%
  ...
  --- using \Option{per-mode=fraction,fraction-function=\Macro{sfrac}}; % \footnote{\Option{per=fraction,fraction=nice} in \TeXLive 2009}
...
%   \end{equation*}
% }
with \Option{per-mode=symbol}; % \footnote{\Option{per=slash} in \TeXLive 2009};
