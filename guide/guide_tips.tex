...
%------------------------------------------------------------------------------
\section{How to use the \Package{pibonn-thesis} style}
...
yourself and are in the \texttt{pithesis} directory.
...
(valid for release 1.3 onwards):
\begin{verbatim}
svn co https://svn.physik.uni-bonn.de/basic/pithesis/tags/pithesis-N.M
\end{verbatim}
\noindent
...
\begin{verbatim}
cd ~/pithesis/trunk
svn switch --relocate svn://svn.physik.uni-bonn.de/pithesis \
 https://svn.physik.uni-bonn.de/basic/pithesis
\end{verbatim}
\noindent
...
should work with \TeXLive 2009.\index{TeXLive@\TeXLive!2009} If you
have a newer version, you should set the value of \Macro{texlive} in
the thesis main file to 2011\index{TeXLive@\TeXLive!2011} rather than
...
\Macro{InstituteAddress}. The style file \texttt{pibonn-thesis.sty}
...
\texttt{pibonn-thesis.sty} to the font you actually used in your
...
should compile without changes for \TeXLive 2009. For later versions
set \Macro{texlive} in \texttt{thesis\_guide.tex}.\footnote{You should
  set \Macro{texlive} to 2011 also if you have \TeXLive
  2012,\index{TeXLive@\TeXLive!2012} which is the default version for
  Ubuntu 12.10.\index{ubuntu!12.10}}
...
\item If you use normal words in superscripts or subscripts (or
  ...
  because the
  \Macro{sisetup}\texttt{\{obeyall\}}\footnote{\Option{detect-family=true}
    in \TeXLive 2011.} option is set in \texttt{pibonn-thesis.sty}
  (see Section~\ref{sec:tips:siunitx}).  I used the option
  \Option{obeyfamily=false}\footnote{\Option{detect-family=false} in \TeXLive
    2011} for the \Macro{SI} command in the 2nd expression.
...
\item Use \Macro{enquote} from the \PackageG{csquotes} package to
  ...
  (and dialect\footnote{%
    ...
    (and in \texttt{pibonn-thesis.sty})
...
\Option{version-1-compatibility}. As \TeXLive 2009 was the default
version for Ubuntu releases up to and including
12.04,\index{ubuntu!12.04} I will give the \Package{siunitx} version 1
options in the text and the version 2 options as footnotes.

One very attractive feature is
...
\Option{expproduct}\footnote{\Option{exponent-product} in \TeXLive
  2011.} you can also get
\ifthenelse {\texlive = 2009} {%
...
\Option{dp}\footnote{\Option{round-mode} and
  \Option{round-precision} in \TeXLive 2011} option, and even rounds
correctly!
...
\Option{seperr}\footnote{\texttt{separate-uncertainty}
  in \TeXLive 2011.} option (which I specify) or $2.88(32)$ with the
...
\ifthenelse {\texlive = 2009} {%
  ...
  -- using \Option{per=fraction,fraction=nice}\footnote{%
    \Option{per-mode=fraction,fraction-function=\Macro{sfrac}} in
      \TeXLive 2011}
...
with \Option{per=slash}\footnote{%
  \Option{per-mode=symbol} in \TeXLive 2011};
...
\Macro{sisetup}\texttt{\{obeyall\}}\footnote{\texttt{detect-family=true}
  in \TeXLive 2011.} to ensure that the package uses the current font
...
\item You are required to include a CV\index{CV} and a summary of your
